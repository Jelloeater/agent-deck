...
|---|---|---|
| `unit-tests.yml` | PR touching `cmd/agent-deck/**`, `internal/**`, `go.mod`/`go.sum`, or the workflow itself; also `workflow_dispatch` | `go test -race -count=1 ./cmd/agent-deck/...` with the race detector. Catches regressions from new CLI commands (e.g. `completions`, `session`, `mcp`) before a tag is cut. Mirrors the test step in `release.yml` but fires on PRs instead of only at tag-push time. |
